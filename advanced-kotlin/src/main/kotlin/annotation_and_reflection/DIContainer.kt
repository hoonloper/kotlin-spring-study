...
fun start(clazz: KClass<*>) {
    val reflections = Reflections(clazz.qualifiedName)
...
fun main() {
    ...
    // ContainerV1.register(BService::class)

    // val aService = ContainerV1.getInstance(AService::class)
    // val bService = ContainerV1.getInstance(AService::class)

    // aService.print()
    // bService.print()

    // val reflections = Reflections("annotation_and_reflection")
    // val jClasses = reflections.getTypesAnnotatedWith(MyClass::class.java)
    // println(jClasses)

    start(DIContainer::class)
    val bService = ContainerV2.getInstance(BService::class)
    println(bService.print())
}
